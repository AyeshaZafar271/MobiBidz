...
var pid = document.getElementById("product_id").innerText;
var pname = document.getElementById("product_name").innerText;
var price = document.getElementById("product_price").innerText;
console.log(pid);
...
async function initialize() {
    const { id, clientSecret } = await fetch("payment_init.php", {
        ...
        body: JSON.stringify({ request_type:'create_payment_intent' , product_id: pid, product_name: pname, product_price: price  }),
    }).then((r) => r.json());
...
async function handleSubmit(e) {
    ...
    const { id, customer_id } = await fetch("payment_init.php", {
        method: "GET",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ request_type:'create_customer', payment_intent_id: payment_intent_id, name: customer_name, email: customer_email, product_id: pid, product_name: pname, product_price: price }),
    }).then((r) => r.json());
        ...
        switch (paymentIntent.status) { 
            case "succeeded":
                ...
                fetch("payment_init.php" , {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ request_type:'payment_insert', payment_intent: paymentIntent, customer_id: customerID ,product_id: pid, product_name: pname, product_price: price }),
                })
                .then(response => response.json())
                .then(data => {
                    if (data.payment_txn_id) {
                        window.location.href = 'payment-status.php?pid='+data.payment_txn_id+"&product_id"+product_id+"&product_name="+product_name+"&product_price="+product_price";
                    } else {
