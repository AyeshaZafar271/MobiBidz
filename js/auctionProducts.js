...
function populateData() {
    ...
    xmlhttp.open("GET", "getAuctionProductDataController.php", true);
    xmlhttp.send();
...
function displayProducts(products) {
  if (products.length > 0) {
    const product_details = products
      .map(
        (product) => `
  <div onClick="document.location.href='auctionDetails.php?productId=${product.id}&product_price=${product.amt}'"  class="product">
  <div class="img">
